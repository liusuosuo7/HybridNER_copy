...
class BERTNERDataset(Dataset):
	...
	def __getitem__(self, item):
		...
		# 中文数据集处理
		if self.dataname in ['msra', 'weibo', 'ontonotes4', 'cluener', 'cmeee']:
			# 对于中文数据集，按字符分割
			words = list(context.replace(' ', ''))
		else:
			# 英文数据集，按空格分割
			words = context.split()
...
def get_loader(args, data_dir, prefix, shuffle, limit: int = None):
	...
	# 检查是否为中文数据集
	if args.dataname in ['msra', 'weibo', 'ontonotes4', 'cluener', 'cmeee']:
