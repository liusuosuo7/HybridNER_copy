...
def get_args():
    # Use conflict_handler='resolve' to avoid errors if the same option is
    # accidentally added more than once (e.g., during iterative development).
    parser = argparse.ArgumentParser(description="Training", conflict_handler='resolve')
    ...
    parser.add_argument(
        "--dataname",
        choices=["conll03", "notev4", "wnut17", "twitter", "bioner", "msra", "weibo", "ontonotes4", "navigation"],  # 加入 navigation
        default="msra",
        type=str,
        help="the name of a dataset"
    )
    parser.add_argument("--data_dir", default="/root/autodl-tmp/HybridNER/dataset/msra", type=str, help='The file path where the dataset is located')
    ...
    parser.add_argument('--etrans_func', default='exp', type=str,
                        help='Type of evidence used in training, choice in [softplus, exp, relu, softmax]')
    ...
    parser.add_argument('--with_uc', type=str2bool, default=True,
                        help='Whether to use regularization in the loss function')
    parser.add_argument('--with_iw', type=str2bool, default=True,
                        help='Whether to use reweighting in the loss function')
    parser.add_argument('--with_kl', type=str2bool, default=True,
                        help='Whether to use Kullback-Leibler divergence in the loss function')
    ...
    # BERT模型相关参数
    parser.add_argument("--bert_config_dir", type=str, default='/root/autodl-tmp/HybridNER/models/bert-base-cased',
                        help="BERT configuration directory")
    ...
    parser.add_argument("--final_div_factor", type=float, default=1e4,
                        help="Final division factor for linear decay scheduler")
    ...
    parser.add_argument("--data_sign", type=str, default="zh_msra", help="Data signature for the dataset")
    ...
    parser.add_argument('--use_tokenLen', default=True, type=str2bool,
                        help='Use token length after BERT tokenizer process as a feature', nargs='?',
    ...
    # --------------------- Combination/Ensemble Settings -------------------
    # 组合器参数：改为开关型 action='store_true'，以兼容 "--use_combiner" 不带值的用法
    parser.add_argument('--use_combiner', action='store_true', help='是否启用组合器功能')
    parser.add_argument('--comb_model_results', type=str, nargs='*', default=[], help='待组合的模型结果文件路径列表')
    parser.add_argument('--comb_prob_files', type=str, nargs='*', default=None,
                        help='与comb_model_results一一对应的概率pkl文件路径列表；可留空用默认0.5')
    parser.add_argument('--comb_strategy', type=str, default='all',
                        choices=['majority', 'weighted_f1', 'weighted_cat', 'span_score', 'all'],
                        help='组合策略选择：majority(多数投票), weighted_f1(整体F1加权), weighted_cat(类别F1加权), span_score(Span分数), all(全部策略)')
    parser.add_argument('--comb_result_dir', type=str, default='comb_result', help='组合结果保存目录')
    parser.add_argument('--auto_combine_after_train', action='store_true', help='训练后是否自动运行组合器')
    parser.add_argument('--auto_combine_after_inference', action='store_true', help='推理后是否自动运行组合器')

    # --------------------- Combination/Ensemble Settings -------------------
    # （保留你原来的第二块，修正弯引号与 type）
    parser.add_argument('--use_combiner', action='store_true', help='是否启用组合器功能')
    parser.add_argument('--comb_model_results', type=str, nargs='*', default=[], help='待组合的模型结果文件路径列表')
    parser.add_argument('--comb_strategy', type=str, default='all',
                        choices=['majority', 'weighted_f1', 'weighted_cat', 'span_score', 'all'],
    ...
    # 可调组合权重（用于Span预测分数投票）
    parser.add_argument('--comb_wscore', type=float, default=0.8, help='Span分数权重，默认0.8，越大越依赖置信度')
    parser.add_argument('--comb_wf1', type=float, default=1.0, help='整体F1权重，默认1.0，越大越依赖模型整体F1')

    # 当需要自动生成span概率文件时，指定用于打分的测试集目录与模型路径
    parser.add_argument('--comb_dataset_dir', type=str, default='dataset/conll03', help='用于生成span概率的测试集目录')
    parser.add_argument('--comb_span_model', type=str, default=None,
                        help='用于生成span概率的已训练模型路径，若不设则尝试使用 --inference_model 或 best_model.pkl')
